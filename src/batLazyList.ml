...
let rev list = fold_left (fun acc x -> Lazy.from_val (Cons (x, acc))) nil list

(**Revert a list, convert it to a lazy list.
   Used as an optimisation.*)
let rev_of_list (list:'a list) = List.fold_left (fun acc x -> Lazy.from_val (Cons (x, acc))) nil list
...
let rev_append (l1 : 'a t) (l2 : 'a t) =
  let rec aux list acc =
    match next list with
    | Cons (x, t) -> aux t (Lazy.from_val (Cons (x, acc)))
    | Nil -> acc
  in aux l1 l2
...
*)
let eager_of_list l =
  ListLabels.fold_right ~init: nil ~f: (fun x acc -> Lazy.from_val (Cons (x, acc))) l

(**
   Eager conversion from array
*)
let of_array l =
  ArrayLabels.fold_right ~init: nil ~f: (fun x acc -> Lazy.from_val (Cons (x, acc))) l
